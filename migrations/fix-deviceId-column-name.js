// migrations/fix-deviceId-column-name.js

module.exports = {
  up: async (queryInterface, Sequelize) => {
    const tableName = 'devices'; // ✅ Correct name: lowercase
    ...
    try {
      ...
      throw new Error(`Migration aborted: table "devices" does not exist.`);
    }

    // Rename deviceId → device_id (if needed)
    if (tableInfo.deviceId && !tableInfo.device_id) {
      console.log('✅ Renaming column `deviceId` → `device_id`');
      await queryInterface.renameColumn(tableName, 'deviceId', 'device_id');
    } else if (tableInfo.device_id) {
      console.log('⏭️ Column `device_id` already exists. Skipping rename.');
    }

    // Optionally handle other columns
